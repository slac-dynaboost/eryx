...
class TestModeEquivalence(unittest.TestCase):
    ...
    def _run_and_compare(self, use_data_adp: bool):
        ...
        # --- 2. Arbitrary Mode Run (Separate Instance) ---
        print("\nInitializing arbitrary-q model...")
        # Use the q_grid generated by the grid model as input
        q_vectors_from_grid = model_grid.q_grid.clone().detach()
        model_q = TorchOnePhonon(
            pdb_path=self.pdb_path,
            q_vectors=q_vectors_from_grid, # Use q_grid from grid model
            hsampling=self.hsampling, # Still needed for ADP calc if model='gnm'
            ksampling=self.ksampling,
            lsampling=self.lsampling,
            device=self.device,
            **self.common_params
        ...
        # --- 3. Initial State Assertions (Optional but Recommended) ---
        print("\nAsserting initial state consistency...")
        # Compare q_grid (should be identical)
        self.assertTrue(torch.allclose(model_grid.q_grid, model_q.q_grid, rtol=1e-7, atol=1e-9), "q_grid differs between models")
        # Compare hkl_grid (should be identical)
        self.assertTrue(torch.allclose(model_grid.hkl_grid, model_q.hkl_grid, rtol=1e-7, atol=1e-9), "hkl_grid differs between models")
        # Compare resolution mask (should be identical)
        self.assertTrue(torch.equal(model_grid.res_mask, model_q.res_mask), "res_mask differs between models")
        # Compare computed ADP if applicable
        if not use_data_adp and self.common_params.get('model') == 'gnm':
            self.assertTrue(hasattr(model_grid, 'ADP') and model_grid.ADP is not None, "Grid model missing ADP")
            self.assertTrue(hasattr(model_q, 'ADP') and model_q.ADP is not None, "Arbitrary model missing ADP")
            self.assertTrue(torch.allclose(model_grid.ADP, model_q.ADP, rtol=self.rtol, atol=self.atol), "Computed ADP differs between models")
        # Compare Amat (should be identical)
        self.assertTrue(torch.allclose(model_grid.Amat, model_q.Amat, rtol=1e-7, atol=1e-9), "Amat differs between models")
        print("Initial state assertions PASSED.")

        # --- 4. Comparison ---
        print("\nComparing Grid vs Arbitrary-Q results...")
